...
Dirbtiniai neuroniniai tinklai susideda iš dirbtinių neuronų. Tai trijų sluoksnių tinklas, kuris susideda iš įeigos (\textit{angl. input}), paslėptojo (\textit{angl. hidden}), kuris gali būti sudarytas iš vieno ar daugiau sluoksnių, ir išeigos (\textit{angl. output}) sluoksnių.
...
Kiekvienas neuronas dirbtiniuose neuroniniuose tinkluose gali gauti vieną ar kelias įeigas. Visų šių įeigų svorių suma yra apdorojama. Ji apdorojama aktyvacijos funkcija. Yra keletas skirtingų aktyvacijos funkcijų, kaip, pavyzdžiui, tiesinė, \textit{tanh}, \textit{ReLU} ir nelinijinės regresijos funkcijos. Toliau, \ref{eq:relu} formulėje pateikiamas \textit{ReLU} aktyvacijos funkcijos principas.
